...
* **Manga Navigation:** Right-to-Left navigation (Left arrow advances the story).
* **Smart Scaling:** Automatically maximizes and fits pages to the screen while maintaining aspect ratio.
...
### macOS (Homebrew)
```bash
brew install sdl2 sdl2_image libzip
```
### Linux (Debian/Ubuntu)
```bash
sudo apt-get install build-essential libsdl2-dev libsdl2-image-dev libzip-dev
```
### Windows (MSYS2 MinGW 64-bit)
```bash
pacman -S mingw-w64-x86_64-gcc mingw-w64-x86_64-make mingw-w64-x86_64-SDL2 mingw-w64-x86_64-SDL2_image mingw-w64-x86_64-libzip
